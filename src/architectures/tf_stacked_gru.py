...
def build_model(config, lookback, n_price_features, n_macro_features):
    ...
    # Price stream
    x_price = GRU(units_price_1, return_sequences=True)(price_input)
    x_price = Dropout(dropout_price_1)(x_price)
    x_price = GRU(units_price_2, return_sequences=True)(x_price)
    x_price = Dropout(dropout_price_2)(x_price)
    x_price = GRU(units_price_3, return_sequences=True)(x_price)
    x_price = LayerNormalization()(x_price)

    # Macro stream
    x_macro = GRU(units_macro_1, return_sequences=True)(macro_input)
    x_macro = Dropout(dropout_macro)(x_macro)
    x_macro = GRU(units_macro_2, return_sequences=True)(x_macro)
